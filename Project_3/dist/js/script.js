...
document.addEventListener('DOMContentLoaded', () => {
  ...
  slidesImg.forEach(slide => {
    // перебор всех слайдов,и установка всем одинаковой ширины
    slide.style.width = width;
  });
  sliderNextBtn.addEventListener('click', () => {
    if (offset == +width.slice(0, width.length - 2) * (slidesImg.length - 1)) {
      // когда дойдет до последнего слайда =Ю сбрасываем отступ
      offset = 0;
    } else {
